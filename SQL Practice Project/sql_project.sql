...
SELECT m.firstname, m.surname FROM `Members` m 
INNER JOIN 
(SELECT MAX(joindate) last_joindate FROM `Members`) AS t 
ON m.joindate=t.last_joindate
...
WHERE name LIKE 'Tennis%' AND m.memid !=0
